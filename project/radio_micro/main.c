...
/*                电脑串口发送：1122334455             */
/* 串口波特率：9600                                    */        
/* 作者：ration     （论坛：www.rationmcu.com）        */
/*                  （店址：ration.taobao.com）        */
/* 联系方式：QQ:253057617     旺旺：qq253057617        */
/*           手机：15234021671（山西太原）             */
/*******************************************************/
#include<reg51.h>
#include<intrins.h>
...
//		NRF24L01_Write_Reg(FLUSH_RX,0xff);//清除RX FIFO寄存器
		...
		if (morePayload & 0x01)	{
			...
			*again = 1;
			return 2; // 有更多数据等待读取，需要再次读FIFO寄存器
		}
	}	   
	return 1;//没收到任何数据
}
...
{
	AUXR |= (1<<6);     // 设置定时器1为1T模式
	TMOD = 0x20;				// 定时器T1使用工作方式2,8位自动重载
	SCON = 0x50;				// SM0=0 SM1=1 SM2=0 REN=1 TB8=0 RB8=0 TI=0 RI=0
	TH1 = 0xFD;					// 9600	(外接11.0592M晶振)
	TL1 = 0xFD;
	PCON = 0x80;				// SMOD =1    倍频
	TR1 = 1;					// 开始计时
}
...
{	 
	...
	if(RI)
    {
		rece_buf[j++]=SBUF;	  //接收到的数据放入缓冲区recebuf中
		RI=0;
		while(i<500) 
		{
		  	if(RI)
...
//  	NRF24L01_Write_Reg(WRITE_REG+SETUP_RETR,0x1a);//设置自动重发间隔时间:500us + 86us;最大自动重发次数:10次
  	...
  	NRF24L01_Write_Reg(WRITE_REG+RF_SETUP,0x26);  //设置TX发射参数,0db增益,250kbps,低噪声增益开启   
