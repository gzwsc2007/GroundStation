...
Generated from: aeroInfoSystem.xml
...
class MAVLink_pfd_message(MAVLink_message):
        ...
        Flight Display. Should be sent at 10Hz.
        '''
        def __init__(self, roll, pitch, yaw, altitude, airspeed):
                MAVLink_message.__init__(self, MAVLINK_MSG_ID_PFD, 'PFD')
                self._fieldnames = ['roll', 'pitch', 'yaw', 'altitude', 'airspeed']
                self.roll = roll
                self.pitch = pitch
                self.yaw = yaw
        ...
        def pack(self, mav):
                return MAVLink_message.pack(self, mav, 21, struct.pack('<hhhhh', self.roll, self.pitch, self.yaw, self.altitude, self.airspeed))
...
class MAVLink_navd_message(MAVLink_message):
        ...
        '''
        def __init__(self, battV, battI, temp, latitude, longitude, course, groundspeed):
                MAVLink_message.__init__(self, MAVLINK_MSG_ID_NAVD, 'NAVD')
                self._fieldnames = ['battV', 'battI', 'temp', 'latitude', 'longitude', 'course', 'groundspeed']
                self.battV = battV
                self.battI = battI
                self.temp = temp
...
class MAVLink(object):
        ...
        def decode(self, msgbuf):
                ...
                return m
        def pfd_encode(self, roll, pitch, yaw, altitude, airspeed):
                '''
                This message encodes data needed for the GroundStation Primary Flight
                Display. Should be sent at 10Hz.

                roll                      : Roll angle in 0.01 degrees. Positive means rolling CCW. (int16_t)
                pitch                     : Pitch angle in 0.01 degrees. Positive means pitching up. (int16_t)
                yaw                       : Yaw angle (magnetic heading) in 0.01 degrees. Positive means yawing CW. (int16_t)
                altitude                  : altitude (AGL) in meters. (int16_t)
                airspeed                  : airspeed in 0.1 m/s. (int16_t)

                '''
                msg = MAVLink_pfd_message(roll, pitch, yaw, altitude, airspeed)
                msg.pack(self)
                return msg
            
        def pfd_send(self, roll, pitch, yaw, altitude, airspeed):
                '''
                This message encodes data needed for the GroundStation Primary Flight
                Display. Should be sent at 10Hz.
                ...
                '''
                return self.send(self.pfd_encode(roll, pitch, yaw, altitude, airspeed))
            
        def navd_encode(self, battV, battI, temp, latitude, longitude, course, groundspeed):
                '''
        ...
        def navd_send(self, battV, battI, temp, latitude, longitude, course, groundspeed):
                '''
                This message encodes data needed for the GroundStation Navigation
                Display. Should be sent at 5Hz.
